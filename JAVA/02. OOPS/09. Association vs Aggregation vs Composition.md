...
![](images/java_association_aggregation_composition.png)
...
**Q1: What is the main difference between Aggregation and Composition?**
A: **Lifecycle dependency.** In Aggregation, the child can exist without the parent (e.g., Student and University). In Composition, the child is destroyed when the parent is destroyed (e.g., Room and House).

**Q2: When should you use "Composition over Inheritance"?**
A: When you want to combine behaviors from multiple sources (Java doesn't allow multiple inheritance) or when the relationship is not a strict "is-a". Composition is more flexible as you can change the component at runtime.

**Q3: Can we represent Composition in Java using code?**
A: There is no special keyword for composition. We achieve it by creating the "part" object inside the "whole" class (usually in the constructor) and making the reference `private` or `final`.

**Q4: Is a "Has-A" relationship always Composition?**
A: No. "Has-A" is the general term for Association. It could be Aggregation (weak) or Composition (strong).

**Q5: What is the advantage of loose coupling (Association)?**
A: It allows objects to be swapped or modified independently without breaking the entire system.

---

## One-liner to Remember

> **Relationship Strength: Association (Friend) < Aggregation (Team Member) < Composition (Heart).**
