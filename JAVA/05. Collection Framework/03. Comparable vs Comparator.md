...
This is the single method required by the interface. It compares the current object (`this`) with a specified object (`other`).
...
While useful for a default sort order, `Comparable` has three major drawbacks:

1. **Single Ordering:** You can only define **one** natural sort order per class. If you usually sort by `age`, but occasionally need to sort by `name`, `Comparable` cannot handle both.
2. **Modification Required:** You must have access to the source code to implement `Comparable`. You cannot add natural ordering to a library class (like a third-party `User` class) if it wasn't already there.
3. **Rigidity:** Changing the sorting logic requires modifying the class itself, which may break other parts of the application that rely on the original order.
...
The interface requires implementing `compare(T o1, T o2)`, which compares two separate objects:
...
- Use **`Comparable`** if the class has one logical, "natural" way to be ordered that will rarely change.
- Use **`Comparator`** if you need to sort the same list in different ways depending on the UI or business logic, or if you cannot modify the original class.
